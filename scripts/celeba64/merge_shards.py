"""
merge_celeba64_shards.py — Merge shard HDF5 files into one master HDF5 file.

Usage:
    python scripts/celeba64/merge_shards.py
    python scripts/celeba64/merge_shards.py --shard_dir data/celeba64/shards \
        --out_file data/celeba64/celeba64_gaussians_K1000.h5
"""
...
def main():
    parser = argparse.ArgumentParser(description="Merge CelebA-64 Gaussian shard files into one HDF5.")
    parser.add_argument("--shard_dir", default="data/celeba64/shards",
                        help="Directory containing shard_0.h5 … shard_N.h5")
    parser.add_argument("--out_file", default="data/celeba64/celeba64_gaussians_K1000.h5",
                        help="Output merged HDF5 file")
    parser.add_argument("--n_shards", type=int, default=20, help="Number of shards")
    parser.add_argument("--n_total", type=int, default=202599, help="Total images")
    ...
    print(f"\n{'='*68}")
    print(f"  merge_celeba64_shards.py")
    ...
    n_val = int((orig_split_all == 1).sum())
    n_te = int((orig_split_all == 2).sum())
    print(f"  Train={n_tr} Val={n_val} Test={n_te}  (expected 162770/19867/19962)")
    ...
    with h5py.File(out_file, "w") as hf:
        ...
        hf.attrs["channels"] = 3
        hf.attrs["image_size"] = 64
        hf.attrs["crop_size"] = 140
        hf.attrs["dataset"] = "celeba64"
        hf.attrs["lr"] = 0.04
        ...
        hf.attrs["n_train"] = n_tr
        hf.attrs["n_val"] = n_val
        hf.attrs["n_test"] = n_te
    ...
    print(f"\n{_hr()}")
    print(f"  Per-split PSNR")
    print(_hr())
    for split_val, split_name in [(0, "train"), (1, "val"), (2, "test")]:
        mask = orig_split_all == split_val
        psnr_s = psnr_all[mask]
        n_s = len(psnr_s)
        if n_s > 0:
            n_low = int((psnr_s < 40.0).sum())
            print(
                f"  {split_name:>8}  n={n_s:>6}  mean={psnr_s.mean():>7.2f}  "
                f"std={psnr_s.std():>6.2f}  min={psnr_s.min():>7.2f}  "
                f"max={psnr_s.max():>7.2f}  <40dB={n_low:>6}"
            )
    ...
    summary = {
        ...
        "per_split": {
            name: {
                "n": int((orig_split_all == val).sum()),
                "mean": float(psnr_all[orig_split_all == val].mean()),
                "min": float(psnr_all[orig_split_all == val].min()),
            }
            for val, name in [(0, "train"), (1, "val"), (2, "test")]
        }
    }
    summary_path = out_file.parent / "celeba64_gaussians_K1000_summary.json"
