"""
Train GaussianTransformer diffusion model on CelebA-64 Gaussian representations.

CelebA-64 specifics:
  - K=1000 Gaussians, 8-dim features (alpha dropped): sigma_x, sigma_y, rho, r, g, b, x, y
  - 64x64 RGB images, kernel_size=32, soft_clamp=True
  - Unconditional generation (no class labels, no CFG)

Usage:
    python scripts/celeba64/train.py --data_h5 data/celeba64/celeba64_gaussians_K1000.h5 --epochs 500
"""
...
K_DEFAULT = CFG["num_gaussians"]  # 1000


# ---------------------------------------------------------------------------
# Reverse diffusion sampler (unconditional)
# ---------------------------------------------------------------------------
@torch.no_grad()
def ddpm_sample(model, ddpm, n, K, device):
...
def render_sample_grid(W_norm, n_show=16, device="cuda"):
    ...
    grid = make_grid(torch.stack(frames), nrow=4, normalize=False)
    return grid
...
def train(args):
    ...
    # Model (unconditional: num_classes=0)
...
def main():
    parser = argparse.ArgumentParser(description="Train GaussianDiffusion on CelebA-64")
    parser.add_argument("--data_h5", required=True,
                        help="Path to CelebA-64 HDF5 file")
    parser.add_argument("--min_psnr", type=float, default=None)
    parser.add_argument("--checkpoint_dir", default="checkpoints/celeba64/")
    parser.add_argument("--sample_dir", default="samples/celeba64/")
