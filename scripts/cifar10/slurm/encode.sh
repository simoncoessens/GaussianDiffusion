#!/bin/bash
#SBATCH --job-name=encode_cifar10
...
#SBATCH --output=logs/encode_cifar10_%A_%a.out
#SBATCH --error=logs/encode_cifar10_%A_%a.err

# Encode all 60,000 CIFAR-10 images into 12 interleaved shard HDF5 files.
# Each shard processes 5,000 images on one GPU using batched encoding.
# Default batch_size=512 (V100). Use BATCH_SIZE=1024 for A100.
#
# Submit (A100, 4 shards, BS=1024):
#   BATCH_SIZE=1024 sbatch --array=0-3 -p gpua100 scripts/cifar10/slurm/encode.sh
#
# Submit (V100, 8 shards, BS=512):
#   sbatch --array=4-11 -p gpu scripts/cifar10/slurm/encode.sh
#
# Resume after crash:
#   RESUME=1 sbatch --array=2 -p gpua100 scripts/cifar10/slurm/encode.sh
...
cd "$WORKDIR"
mkdir -p logs data/cifar10/shards
...
$PYTHON scripts/cifar10/encode.py \
    --shard_id     "$SHARD_ID" \
    --n_shards     "$N_SHARDS" \
    --out_dir      data/cifar10/shards \
    --device       "$DEVICE" \
    --K            "$K" \
    --epochs       "$EPOCHS" \
    --lr           "$LR" \
