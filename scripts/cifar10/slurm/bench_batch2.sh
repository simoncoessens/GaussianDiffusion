...
#SBATCH --output=logs/bench_batch2_%j.out
#SBATCH --error=logs/bench_batch2_%j.err
...
for i in range(N_TOTAL):
    ...
    if (i+1) % 8 == 0:
        print(f'  [{i+1}/{N_TOTAL}] mean={np.mean(psnrs_seq):.1f}dB')
t_seq = time.time() - t0
print(f'  Total: {t_seq:.1f}s ({t_seq/N_TOTAL:.2f}s/img)  PSNR={np.mean(psnrs_seq):.1f}dB')
print()

for BS in [4, 8, 16, 32]:
    print(f'=== Batch size = {BS} (loss=sum, fixed) ===')
    all_imgs = torch.from_numpy(raw_images[:N_TOTAL]).float() / 255.0
    torch.cuda.reset_peak_memory_stats()
    t0 = time.time()
    all_losses = []
    for start in range(0, N_TOTAL, BS):
        end = min(start + BS, N_TOTAL)
        W_batch, losses = encode_batch(all_imgs[start:end], K=K, epochs=epochs, lr=lr,
    ...
    print(f'  {t_batch:.1f}s ({t_batch/N_TOTAL:.2f}s/img)  PSNR={np.mean(psnrs_batch):.1f}dB  min={np.min(psnrs_batch):.1f}  speedup={speedup:.2f}x  mem={mem:.2f}GB')
    print()
"
