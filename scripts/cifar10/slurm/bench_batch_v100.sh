...
#SBATCH --output=logs/bench_batch_v100_%j.out
#SBATCH --error=logs/bench_batch_v100_%j.err
...
t_seq = time.time() - t0
print(f'  {t_seq:.1f}s ({t_seq/N_TOTAL:.2f}s/img)  PSNR={np.mean(psnrs_seq):.1f}dB')
print()
...
for BS in [4, 8, 16, 32]:
    ...
    print(f'  {t_batch:.1f}s ({t_batch/N_TOTAL:.2f}s/img)  PSNR={np.mean(psnrs_batch):.1f}dB  speedup={speedup:.2f}x  mem={mem:.2f}GB')
