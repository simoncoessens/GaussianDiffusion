...
class InitialProjection(nn.Module):
    def __init__(self, in_dim, final_dim=1024):
        ...
        self.shortcut = nn.Linear(in_dim, final_dim, bias=False)
    ...
    def forward(self, x):
        ...
        out = self.layer3(out)
        out = self.layer4(out)
        sc = self.shortcut(x)  # Directly map (B, L, in_dim) -> (B, L, final_dim)
        return out + sc
...
class OutputProjection(nn.Module):
    def __init__(self, in_dim=1024, out_dim=7):
...
class GaussianTransformer(nn.Module):
    def __init__(self, input_dim, time_emb_dim, feature_dim, num_timestamps, num_transformer_blocks=6, num_heads=8):
        ...
        self.initial_projection = InitialProjection(in_dim=feature_dim, final_dim=1024)
