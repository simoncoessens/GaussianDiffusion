...
from models.pa_mha import ParameterAwareGaussianTransformer
...
T = 200                      # Number of timesteps for DDPM
DEVICE = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

# Build a file tag based on the number of heads and blocks
FILE_TAG = f"PA_MHA_sprites_{NUM_HEADS}h_{NUM_BLOCKS}blocks_{T}ts"
...
def main():
    ...
    if RESUME_TRAINING and os.path.exists(RESUME_CHECKPOINT):
        wandb.init(project='Sprites_PA_MHA',
