...
BATCH_SIZE = 32
NUM_EPOCHS = 1000  # Train for 1000 epochs total
LEARNING_RATE = 1e-3         
...
SAMPLE_EVERY_N_EPOCHS = 5  # Sample every N epochs to save time
...
def train_model(train_loader, val_loader, model, scheduler, optimizer, num_epochs, param_ranges, start_epoch=0, dataset=None):
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        saved_val_best = False
        if val_loss <= best_val_loss:
            best_val_loss = val_loss
            best_val_path = save_model(model, optimizer, epoch, val_loss, save_path=SAVE_PATH, 
        ...
        # Sample and calculate metrics every N epochs or when we have a new best model
        if (epoch + 1) % SAMPLE_EVERY_N_EPOCHS == 0 or saved_val_best:
            print(f"Sampling images at epoch {epoch + 1}...")
            
            # Use the best validation model if we just saved one, otherwise use current model
            eval_model = model
            if saved_val_best:
                print("Using best validation model for sampling")
            else:
                print("Using current model for sampling")
            
            # Sample and plot images
            fig, metrics = sample_and_plot_images(
                eval_model, dataset, param_ranges, DEVICE,
...
def main():
    ...
    if os.path.exists(RUN_ID_FILE):
        with open(RUN_ID_FILE, 'r') as f:
            run_id = f.read().strip()
            print(f"Found existing wandb run ID: {run_id}")
    
    # Check for current epoch
    start_epoch = 0
    if os.path.exists(CURRENT_EPOCH_FILE):
        with open(CURRENT_EPOCH_FILE, 'r') as f:
            try:
                start_epoch = int(f.read().strip())
                print(f"Found saved epoch: {start_epoch}")
            except ValueError:
                print("Could not parse saved epoch, starting from 0.")
                start_epoch = 0
    
    # Initialize or resume wandb run
    if run_id:
        wandb.init(project='sprites_32x32_MHA', 
                   ...
                   id=run_id, 
                   resume="allow")
        print(f"Resumed wandb run with ID: {run_id}")
    else:
        wandb.init(project='sprites_MHA', 
                   config={"learning_rate": LEARNING_RATE, 
                          "epochs": NUM_EPOCHS, 
                          "batch_size": BATCH_SIZE,
                          "num_heads": NUM_HEADS,
        ...
        with open(RUN_ID_FILE, 'w') as f:
            f.write(run_id)
        print(f"Created new wandb run with ID: {run_id}")
    ...
    # Look for existing checkpoint - prioritize timeout checkpoints, then last checkpoints
    timeout_checkpoint = None
    last_checkpoint = os.path.join(SAVE_PATH, build_filename("last"))
    
    # Look for timeout checkpoints first
    for file in os.listdir(SAVE_PATH) if os.path.exists(SAVE_PATH) else []:
        if file.startswith("timeout_") and file.endswith(".pth"):
            timeout_checkpoint = os.path.join(SAVE_PATH, file)
            break
    
    # Load checkpoint if available
    if timeout_checkpoint and os.path.exists(timeout_checkpoint):
        print(f"Found timeout checkpoint: {timeout_checkpoint}")
        start_epoch = load_checkpoint(model, optimizer, timeout_checkpoint)
    elif os.path.exists(last_checkpoint):
        print(f"Found last checkpoint: {last_checkpoint}")
        start_epoch = load_checkpoint(model, optimizer, last_checkpoint)
    else:
