...
from dataset import GaussianDatasetSprites32x32
...
FILE_TAG = f"SPRITES_MHA_{NUM_HEADS}h_{NUM_BLOCKS}blocks_{T}ts_mha_classic_256bs"

# Build a safe save path that includes the parameterized file tag
SAVE_PATH = os.path.join("models", "Sprites", FILE_TAG)
...
RESUME_CHECKPOINT = "/gpfs/workdir/coessenss/gsplat/src/models/Sprites/SPRITES_MHA_8h_8blocks_200ts_mha_classic_256bs/last_SPRITES_MHA_8h_8blocks_200ts_mha_classic_256bs_e100_ts200_mse_std_scaled.pth"

# Define the wandb run id to resume the same run (set this to your existing run id)
WANDB_RUN_ID = "zrhmyocx"
...
def main():
    ...
    model = GaussianTransformer(
        input_dim=150,  # assuming num_gaussians is 500
        time_emb_dim=32,
        feature_dim=8,
        num_timestamps=T,
