...
FILE_TAG = f"SPRITES_64x64_MHA_{NUM_HEADS}h_{NUM_BLOCKS}blocks_{T}ts_mha_classic"
...
RESUME_CHECKPOINT = "/gpfs/workdir/coessenss/gsplat/src/models/Sprites/SPRITES_MHA_64h_32blocks_200ts_mha_classic/last_SPRITES_MHA_64h_32blocks_200ts_mha_classic_e100_ts200_mse_std_scaled.pth"

# Define the wandb run id to resume the same run (set this to your existing run id)
WANDB_RUN_ID = "4pj1eohc"
...
def main():
    ...
    model = GaussianTransformer(
        input_dim=500,  # assuming num_gaussians is 200
        time_emb_dim=32,
        feature_dim=8,
        num_timestamps=T,
