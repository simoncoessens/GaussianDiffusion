...
SAVE_PATH = 'models/MHA_64h_32_blocks/'
BATCH_SIZE = 32
NUM_EPOCHS = 100
LEARNING_RATE = 1e-4         # Try a lower learning rate
WEIGHT_DECAY = 1e-4          # Weight decay for AdamW
T = 200
SAVE_FILENAME = f'MHA_64h_32_blocks_old_param_e{NUM_EPOCHS}_ts{T}_mse_std_scaled_flash.pth'
...
def train_model(train_loader, val_loader, model, scheduler, optimizer, num_epochs, param_ranges):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss <= best_val_loss:
            ...
            save_model(model, optimizer, epoch, val_loss, save_path=SAVE_PATH, filename=f'val_best_MHA_64h_32_blocks_old_param_e{NUM_EPOCHS}_ts{T}_mse_std_scaled.pth')
        if train_loss <= best_train_loss:   
            best_train_loss = train_loss
            save_model(model, optimizer, epoch, train_loss, save_path=SAVE_PATH, filename=f'train_best_MHA_64h_32_blocks_old_param_e{NUM_EPOCHS}_ts{T}_mse_std_scaled.pth')
         # Save intermediate model every 10 epochs
        if epoch + 1 % 10 == 0 or epoch + 1 == 1 :
            intermediate_model_loss = val_loss
            save_model(model, optimizer, epoch, intermediate_model_loss, save_path=SAVE_PATH, filename=f'intermediate_e{epoch+1}_MHA_64h_32_blocks_old_parm.pth')
        save_model(model, optimizer, epoch, val_loss, save_path=SAVE_PATH, filename=f'last_MHA_64h_32_blocks_old_param_e{NUM_EPOCHS}_ts{T}_mse_std_scaled.pth')
...
def main():
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(SAVE_PATH, f"MHA_256h_32_blocks_e{NUM_EPOCHS}_ts{T}_mse_std_scaled.png"))
    plt.show()
