...
from src.dataset import GaussianDatasetSprites32x32
...
def render_latent_to_image(latent: torch.Tensor,
    ...
    if denormalize:
        # Clamp latent vector to ensure it's in valid range before denormalizing
...
if __name__ == "__main__":
    ...
    DATA_FOLDER = "/gpfs/workdir/coessenss/gsplat/data/datasets/Sprites/sprites_results_combined_32x32.h5"
    ...
    # model_path = "/gpfs/workdir/coessenss/gsplat/src/models/Sprites/SPRITES_MHA_64h_10blocks_200ts_mha_classic/train_best_SPRITES_MHA_64h_10blocks_200ts_mha_classic_e100_ts200_mse_std_scaled.pth"
    #model_path = "/gpfs/workdir/coessenss/gsplat/src/models/Sprites/SPRITES_MHA_64h_32blocks_200ts_mha_classic/val_best_SPRITES_MHA_64h_32blocks_200ts_mha_classic_e100_ts200_mse_std_scaled.pth"
    ...
    # model_path = "/gpfs/workdir/coessenss/gsplat/src/models/Sprites/SPRITES_MHA_64h_32blocks_200ts_mha_classic/last_SPRITES_MHA_64h_32blocks_200ts_mha_classic_e100_ts200_mse_std_scaled.pth"
    model_path = "/gpfs/workdir/coessenss/gsplat/src/models/Sprites/SPRITES_MHA_8h_8blocks_200ts_mha_classic_256bs/intermediate_SPRITES_MHA_8h_8blocks_200ts_mha_classic_256bs_e410_checkpoint.pth"
    betas = (1e-4, 0.02)
    n_T = 200
    ...
    dataset = GaussianDatasetSprites32x32(DATA_FOLDER)
    ...
    # Sample latent representations using the diffusion model in batches.
    n_sample = 2000
    batch_size = 256
    num_batches = math.ceil(n_sample / batch_size)
    final_latents = []

    print(f"Generating {n_sample} samples in {num_batches} batches of size {batch_size}...")

    for batch_idx in range(num_batches):
        current_batch_size = min(batch_size, n_sample - batch_idx * batch_size)
        print(f"Processing batch {batch_idx+1}/{num_batches} with {current_batch_size} samples...")
        ...
        final_latents.append(x_history[-1])  # Append only the final latent from each batch

    # Concatenate all batches into one tensor
    final_latents = torch.cat(final_latents, dim=0)
    
    # Calculate min and max values for each of the 8 features
    feature_mins = final_latents.min(dim=0)[0].min(dim=0)[0]  # Min across samples and Gaussians
    feature_maxs = final_latents.max(dim=0)[0].max(dim=0)[0]  # Max across samples and Gaussians
    
    # Print min and max values for each feature
    print("Feature statistics across all samples:")
    for i in range(8):
        print(f"Feature {i}: min = {feature_mins[i].item():.4f}, max = {feature_maxs[i].item():.4f}")
    print(f"Generated {final_latents.shape[0]} samples with shape {final_latents.shape}")

    # Set output folder based on the model name.
    # model_name = os.path.basename(os.path.dirname(model_path))
    model_name = f"Sprites_MHA_8h_8blocks_{n_T}ts_classic"
    ...
    # ----------------------------------------
    n_orig = 2000 # Number of original samples for metrics.
    original_images = []
    
    # Generate random indices to sample from the dataset
    random_indices = torch.randperm(len(dataset))[:n_orig]
    
    for i in range(n_orig):
        idx = random_indices[i].item()
        latent = dataset[idx]  # Get random sample from dataset
